...
interface CommandInputWrapperProps extends React.ComponentPropsWithoutRef<'div'> {
    after?: React.ReactNode;
    before?: React.ReactNode;
}

const CommandInputWrapper = React.forwardRef<React.ElementRef<'div'>, CommandInputWrapperProps>(
    ({ className, children, after, before, ...props }, ref) => (
        <div
            ref={ref}
            ...
            {...props}
        >
            {before && <div className="mr-2">{before}</div>}
            {children}
            {after && <div className="ml-2">{after}</div>}
        </div>
    ),
);
